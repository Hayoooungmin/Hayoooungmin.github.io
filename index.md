...
- 좌측 하단 : 플레이어의 Hp를 확인할 수 있는 UI
- 중앙 상단 : 지구온난화 수치가 얼마인지 확인할 수 있는 UI

## 컨셉 기반 : 배경 및 요소
1. 아기자기한 손맵(스타일라이즈) 그래픽
...
1. 지구온난화와 기후환경에 해로운 각종 대기 오염 물질과 인간과의 전투.

2. 스테이지 내에서 획득 가능한 병, 캔, 고철, 플라스틱을 수집하여 펭귄마을 재활용 센터에서 아이템으로 교환.

3. 분리수거를 통해 펭귄마을의 지구온난화 수치의 상승과 하락.
...
3. 수집한 쓰레기들을 재활용하여 스토리 진행

4. 퀘스트와 스토리를 진행하면 새로운 스테이지에서 다양한 전투맵을 추가
...
1인칭 카메라 시점으로 게임을 진행하여, 실제 플레이어의 시점을 활용한 지구온난화 극복 방법을 명확히 보여준다.
...
스타일라이즈 그래픽을 활용해 남극을 배경으로 한 펭귄마을과 스테이지들을 플레이
...
전투 스테이지에서 전투 시 잔잔하지만 긴장감은 줄 수 있는 브금으로 마을 분위기와 상반되는 느낌을 구현

<br>

## 4. 기술

Unity Engine과 Blender. 3ds max, Zbrush, Substance 3D Painter, Substance 3D Sampler를 활용해 게임을 제작
...
|6|이산화탄소|<img src="./img/이산화질소.PNG" height="300">|
|7|인간|<img src="./img/인간쓰레기.PNG" height="300">|
|8|보스|<img src="./img/보스.PNG" height="300">|
...
### 2. 오브젝트 이름 : 몬스터(메탄가스, 이산화탄소, 인간, 보스)
...
### 3. 오브젝트 이름 : UI(인벤토리, 재활용품 보관, 지구온난화 수치)
...
|속성|영문명칭|설명|비고|
|:----:|:----:|:----:|:----:|
|재활용 성공|Re_Succes|재활용에 성공했을 때 지구온난화 수치가 하락한다.||
|재활용 실패|Re_Fail|재활용에 실패했을 때 지구온난화 수치가 상승한다.||
...
|공격|무기 보유 중에 마우스 왼쪽 누를 시 무기를 휘두르며 공격|
|상호작용|E키 입력 시 주민들과의 퀘스트나 대화 진행|
|아이템 줍기|E키 입력 시 바닥에 떨어져 있는 재활용품이나 쓰레기를 줍는다.|
|아이템 버리기|G키 입력 시 인벤토리에 있는 재활용품을 버린다.|
...
|기본 상태(Idle)|공격|무기를 들고 있는 상태에서 마우스 좌클릭 입력|
...
12. 플레이어는 마우스 좌클릭으로 공격을 한다. 공격은 한 번 눌렀을 때마다 한 번 공격한다.
13. 몬스터는 총 4가지 상태가 있는데, 평상시 상태의 ‘순찰’, 플레이어가 범위에 들어왔을 때 공격하는 ‘공격’, 플레이어의 공격에 맞았을 때 나타나는 ‘피격’, 체력이 모두 달아서 사망할 때 나타나는 ‘사망’이다.
14. 화면 좌측 상단에 인벤토리 UI를 배치한다.
15. 화면 중앙 상단에 지구온난화수치 UI를 배치한다.
16. 지구온난화수치가 100을 초과할 경우 게임오버화면으로 이동한다.
17. 주인공은 두발로 서있고 옷을 입지 않은 상태인 세븐일레븐의 마스코트인 브니를 모티브로 북극곰을 제작한다.
18. 촌장 펭귄 도널드는 빨간색 체크무늬인 헌팅캡을 쓰고 있고 w모양의 콧수염과 검은색 안경을 쓰고 눈썹의 꼬리가 밑으로 처지게 하여 슬픈표정을 짓고 있게끔 제작한다.
19. 괴짜 펭귄 포모나는 빨간색 안경과 마술사 모자를 쓰고 있고 눈은 x로 표시해 괴짜 느낌이 나게끔 제작한다.
20. 마을 주민들은 아기 펭귄, 일반 펭귄, 모자 쓰고 있는 펭귄 3가지로 분류된다.
21. 일반 펭귄은 갈매기 눈썹과 동그랗고 검은 눈을 가졌으며 아무것도 걸치지 않았고, 남색피부와 가슴과 배, 얼굴은 하얗게 제작한다.
22. 모자 쓰고 있는 펭귄은 파란색 캡모자를 쓰고 있고 외형은 일반 펭귄과 동일하게 제작한다,
23. 아기 펭귄은 일반 펭귄처럼 아무것도 걸치지 않았으며 키가 작고 옆으로 뚱뚱하고 외형은 일반 펭귄과 동일하게 제작한다.
24. 몬스터는 메탄가스, 이산화탄소, 인간, 보스 4가지로 분류된다. 
25. 메탄가스의 외형은 분자구조를 외형으로 가진다. 눈과 입은 포켓몬스터의 만화 중 고오스라는 포켓몬의 눈을 모티브하여 제작한다.
26. 이산화질소의 외형은 분자구조를 외형으로 가진다, 눈은 포켓몬스터의 만화 중 고오스라는 포켓몬의 눈을 모티브하여 제작한다.
27. 이산화탄소의 외형은 한점의 하얀색 구름이며 입에는 노란색 번개 모양이 있다. 눈은 포켓몬스터의 만화 중 고오스라는 포켓몬의 눈을 모티브 하여 제작한다.
28. 인간은 대머리에 턱수염이 길게 나있고 빨간 코와 찐한 눈썹, 아래로 긴 동그란 눈을 제작. 인간은 군인들이 입는 전투복을 입고 있으며 허리에 갈색 버클 벨트를 차고 있고, 버클 양쪽으로 포켓이 하나씩 있다. 신발은 갈색으로 아무 무늬 없이 갈색 신발모양으로 제작한다.
29. 마을과 스테이지들의 바닥과 풍경은 빙하 풍경으로 제작한다.
30. 캐나다 시골에 있는 오두막 느낌으로 펭귄 마을의 집들을 문 하나와 창문 하나가 있고, 1층 오두막집으로 제작한다.
31. 촌장 도널드의 집은 마을 왼편 가운데에 제작, 마을에 다른 펭귄들과 동일한 집 텍스쳐를 가졌고, 펭귄 주민들보다 집 크기가 크며, 옛날 서부시대 느낌의 집 외관구조를 가졌다. 
32. 괴짜 펭귄의 집은 마을 외곽에 강을 건너면 있고, 다락방이 있는 2층 구조의 집과 집 밖에는 대장간이 있고, 그 대장간에는 검, 방패 등이 장식되어있다. 그리고 집과 대장간 주변에는 나무 울타리가 둘러 쌓여있다.
33. 재활용센터의 외관은 공장이며 두개의 건물이 붙어있는 형식으로 되있다. 
...
34. 전투맵 1은 주변이 큰 빙하로 덮여있고 안쪽으로 들어가보면 낡고 부서져있는 신전이 하나있다. 그 신전 가운데에 있는 직사각형의 바위위에는 스토리 진행에 필요한 아이템이 놓여져 있다.
35. 전투맵 2는 바다가 있으며 땅에는 빙하로 덮여있다. 해변에는 큰 난파선이 기울어져서 놓여있고, 물가에는 나룻배 2척이 놓여져있다. 왼편 나룻배에는 스토리 진행에 필요한 아이템이 놓여져 있다.
36. 재활용품은 플라스틱, 고철 더미, 쓰레기, 병 4가지로 분류된다. 
37. 고철 더미는 회색과 갈색이 섞여있는 울퉁불퉁한 직사각형으로 제작한다.
38. 플라스틱은 육각형의 파란색 병 모양이며 뚜껑 부분의 색깔만 진하게 제작한다.
39. 쓰레기는 육각형의 검은색 봉투로 되어있고 위부분은 상투머리처럼 튀어나와있고 상투머리 부분과 몸 부분을 잇는 하얀색 끈 같은게 묶여있게끔 제작한다.. 
40. 병은 육각형의 맥주병으로 제작한다.
41. 화면 우측 하단에 호감도 수치 UI를 배치한다.
<br>
...
3. ~~펭귄NPC들과 대화가 가능하고, 퀘스트를 받기 위해서는 해당 펭귄에게 다가가야한다. (7)~~
